...
SQL_QUERY;
    ...
    $rs = exec_query($sql, $query, array($domain_id, $sub_name));
...
function check_subdomain_data(&$tpl, &$sql, $user_id)
{
  ...
  if (isset($_POST['uaction']) && $_POST['uaction'] === 'add_subd') {
    ...
    $sub_name = get_punny(strtolower($_POST['subdomain_name']));

    if (isset($_POST['subdomain_mnt_pt']) && $_POST['subdomain_mnt_pt'] !== '') {
      $sub_mnt_pt = decode_idna(strtolower($_POST['subdomain_mnt_pt']));
    }
...
// dynamic page data.
//
check_subdomain_permissions($sql, $_SESSION['user_id']);
