...
// Check ids the domain already exist
function vhcs_domain_exists ($domain_name)
{
    global $sql;
...
SQL_QUERY;
	...
	for($i=0; $i < count($split_domain) -1; $i++){
		...
		$res2 = exec_query($sql, $query_domain, array($idom));
		$res3 = exec_query($sql, $query_alias, array($idom));
	...
	if ($res_domain -> RowCount() == 0 && $res_aliases -> RowCount() == 0 && $error == 0) {
